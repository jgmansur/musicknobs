...
function gastos_renderLista(append) {
    ...
    page.forEach(row => {
        ...
        const fechaStr = row.fechaCreacion ? formatFechaCreacion(row.fechaCreacion) : (row.fecha ? formatFecha(row.fecha) : '');
...
function gastos_abrirModal(row) {
    ...
    document.getElementById('g-m-fecha').innerText = row.fechaCreacion ? formatFechaCreacion(row.fechaCreacion) : (row.fecha ? new Date(row.fecha).toLocaleDateString('es-MX', { weekday:'short', day:'numeric', month:'long', year:'numeric' }) : '—');
...
function gastos_editarDesdeModal() {
    ...
    const fechaField = document.getElementById('g-fecha-creacion-field');
    const fechaInput = document.getElementById('g-fecha-creacion');
    if (fechaField && fechaInput) {
        fechaField.classList.remove('hidden');
        if (row.fechaCreacion) {
            try {
                const d = new Date(row.fechaCreacion);
    ...
    try {
        ...
        if (nowPartPaid && !options.skipControlLog) {
            ...
            if (item.periodicidad === 'Cuota de Deuda') {
                ...
                if (deuda) {
                    const newMonto = Math.max(0, deuda.monto - monto);
            ...
            try {
                ...
                if (foundRowIndex !== -1) {
                    ...
                    if (item.periodicidad === 'Cuota de Deuda') {
                        ...
                        const deuda = deudasState.allItems.find(d => d.concepto === baseConcept);
                        if (deuda) {
                            const newMonto = deuda.monto + partAmountOriginal;
                            let sheetName = 'Deudas';
                            try {
                                await sheetsGet(SPREADSHEET_DEUDAS_ID, 'Deudas!A1:A1');
                            } catch(e) {
...
function propiedades_bindEvents() {
    ...
    document.getElementById('prop-open-sheet')?.addEventListener('click', () => {
        ...
        propiedades_openSheet(selected?.id || null, 'general');
    });
...
function propiedades_setSheetSection(section = 'all') {
    const cards = [...document.querySelectorAll('#prop-sheet .prop-sheet-card')];
    cards.forEach((card) => {
        const cardSection = card.dataset.section || '';
        const show = section === 'all' || !section || cardSection === section;
        card.classList.toggle('hidden', !show);
    });
}

function propiedades_openSheet(id, section = 'all') {
    ...
    const sectionLabel = {
        all: 'Propiedad',
        general: 'Datos y valuación',
        docs: 'Documentos y enlaces',
    ...
    propiedades_setSheetSection(section);
...
const DOCS_PROFILE_DEFAULTS = [
    { member: 'yo', name: 'Juan G Mansur Gonzalez', birthDate: '1979-05-08', birthWeight: '', curp: 'MAGJ790508HTSNNN05', passportMx: 'N08029798', passportUs: '642824469', visaUs: '', photoUrl: '', vaccinesJson: '[]', notes: '' },
    ...
    { member: 'mascotas', name: 'Mascotas', birthDate: '', birthWeight: '', curp: '', passportMx: '', passportUs: '', visaUs: '', photoUrl: '', vaccinesJson: '[]', notes: 'Documentos de Tocho, Pachanga y otras mascotas.' },
    { member: 'miscelaneo', name: 'Documentos Miscelaneo', birthDate: '', birthWeight: '', curp: '', passportMx: '', passportUs: '', visaUs: '', photoUrl: '', vaccinesJson: '[]', notes: 'Documentos importantes sin titular especifico.' },
];
...
async function documentos_cargarVista() {
    ...
    try {
        ...
        const upgraded = await documentos_upgradeData();
        if (upgraded) await documentos_loadData();
        documentos_render();
    } catch (e) {
...
async function documentos_upgradeData() {
    ...
    DOCS_PROFILE_DEFAULTS.forEach((def) => {
        const current = byMember.get(def.member);
        if (!current) {
            docsState.profiles.push({ ...def, updatedAt: normalizeDateString(new Date().toLocaleDateString('en-CA')) });
            profilesChanged = true;
            return;
        }
        const merged = { ...current };
        ['name', 'birthDate', 'birthWeight', 'curp', 'passportMx', 'passportUs', 'visaUs', 'photoUrl', 'vaccinesJson', 'notes'].forEach((k) => {
            if (!(merged[k] || '').toString().trim() && (def[k] || '').toString().trim()) {
                merged[k] = def[k];
                profilesChanged = true;
            }
        });
        byMember.set(def.member, merged);
    });
    if (profilesChanged) {
    ...
    if (profilesChanged) {
        ...
        const rows = docsState.profiles.map((p) => [p.member, p.name, p.birthDate, p.birthWeight, p.curp, p.passportMx, p.passportUs, p.visaUs, p.photoUrl, p.vaccinesJson, p.notes, p.updatedAt || normalizeDateString(new Date().toLocaleDateString('en-CA'))]);
    ...
    return docsChanged || profilesChanged;
}
...
async function documentos_syncNewFromDrive() {
    const now = normalizeDateString(new Date().toLocaleDateString('en-CA'));
    const [familyFiles, papaFiles] = await Promise.all([
        driveListFolderFilesRecursive(DOCS_FAMILY_FOLDER_ID).catch(() => []),
        driveListFolderFilesRecursive(DOCS_PAPA_FOLDER_ID).catch(() => []),
    ...
    const rows = newFiles.map((f) => {
        ...
        return [
            ...
            member,
            documentos_stripExt(f.name || ''),
            documentos_guessTypeFromName(f.name || ''),
            `${documentos_memberLabel(member)}, importado`,
            f.path || '',
            '',
            f.webViewLink || '',
            f._folderId,
...
function documentos_render() {
    ...
    if (profile.passportUs) infoRows.push(`<div class="docs-copy" data-copy="${profile.passportUs}">Pasaporte USA: ${profile.passportUs} (click para copiar)</div>`);
    if (profile.visaUs) infoRows.push(`<div class="docs-copy" data-copy="${profile.visaUs}">Visa USA: ${profile.visaUs} (click para copiar)</div>`);
    const vacunasSection = Array.isArray(vacunas) && vacunas.length ? `<div class="field-label">Vacunacion</div>${vacunasHtml}` : '';
    const notesRow = profile.notes ? `<div class="docs-meta">${profile.notes}</div>` : '';
    profileEl.innerHTML = `
...
function formatFechaCreacion(str) {
    if (!str) return '';
...
function deudas_formatLongDate(date) {
...
window.deudas_abrirSplit = function(id) {
    ...
    formInputs.forEach(fid => {
        ...
        // Evitar multiples listeners con onchange
        el.oninput = deudas_calcularSplit;
        el.onchange = deudas_calcularSplit;
    });
    
    document.getElementById('d-split-compound').addEventListener('change', (e) => {
        const wrap = document.getElementById('d-split-rate-wrap');
    ...
    const sheet = document.getElementById('d-split-sheet');
    sheet.classList.remove('hidden');
};

window.deudas_cerrarSplit = function() {
    document.getElementById('d-split-sheet').classList.add('hidden');
};

function deudas_calcularSplit() {
    const monto = parseFloat(document.getElementById('d-split-monto').dataset.monto);
    ...
    const isCompound = document.getElementById('d-split-compound').checked;
    let rateAnual = parseFloat(document.getElementById('d-split-rate').value) || 0;
    
    let pago = 0;
    if (!isCompound || rateAnual === 0) {
        pago = monto / n;
    } else {
        // Compound interest: PMT = P * r * (1 + r)^n / ((1 + r)^n - 1)
        let freqDiv = 12; // mensual
        if (freq === 'semanal') freqDiv = 52;
        if (freq === 'quincenal') freqDiv = 24;
        if (freq === 'bimestral') freqDiv = 6;
        if (freq === 'semestral') freqDiv = 2;
        
        let r = (rateAnual / 100) / freqDiv;
        pago = monto * r * Math.pow(1 + r, n) / (Math.pow(1 + r, n) - 1);
    }
    
    document.getElementById('d-split-preview').innerText = formatCurrency(pago);
    document.getElementById('d-split-preview').dataset.pagoCalc = pago;
    return pago;
}
...
document.getElementById('d-split-sheet-overlay').addEventListener('click', () => {
    deudas_cerrarSplit();
});
...
async function skills_cargarVista() {
    ...
    // Wire sheet close controls once
    const backdrop = document.getElementById('skills-sheet-backdrop');
    const closeBtn = document.querySelector('#skills-sheet button');
...
function skills_populate(skills) {
    ...
    function applyFilters() {
        ...
        const filtered = (_skillsData || []).filter(s => {
            const matchQ = !q || [s.name, s.description, s.trigger, s.howToUse, s.category, s.scope, ...(s.ais||[])].some(v => (v||'').toLowerCase().includes(q));
...
function skills_copyName(name, btn) {
    ...
    function fallback() {
        ...
        ta.focus();
        ta.select();
        try { document.execCommand('copy'); onSuccess(); } catch(e) { /* no-op */ }
        document.body.removeChild(ta);
    }
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(name).then(onSuccess).catch(fallback);
    } else {
        fallback();
    }
}
