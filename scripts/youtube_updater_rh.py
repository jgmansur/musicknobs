...
# Configuration — Roby & Hans Place channel
...
def update_video(youtube, video_id, title, description, tags):
    print(f"[Roby & Hans] Updating video {video_id}...")
    ...
    try:
        ...
        if 'UPDATE_TITLE_NOT_ALLOWED_DURING_TEST_AND_COMPARE' in error_str:
            ...
            try:
                response = youtube.videos().update(
                    part='snippet',
                    body=build_body(current['title'])
                ).execute()
                print(f"Video updated (description+tags only). ID: {response['id']}")
                log_update(video_id, current['title'], description, tags)
                return True
...
if __name__ == "__main__":
    ...
    if "--transcript_only" in sys.argv:
        if len(sys.argv) < 3:
            print("Usage: python3 youtube_updater_rh.py --transcript_only <video_id_or_url>")
            sys.exit(1)
    ...
    if len(sys.argv) < 5:
        print("Usage:")
        print("  python3 youtube_updater_rh.py <video_id_or_url> <title> <description> <tags_csv> [--thumbnail=path]")
        print("  python3 youtube_updater_rh.py --transcript_only <video_id_or_url>")
        print("  python3 youtube_updater_rh.py --auth_only")
        sys.exit(1)
